...
<body>
<c:if test="${empty sessionScope.oldForder.sorders }">
    <!-- 如果购物车中的购物项为空，则跳转到首页 -->
        ...
        <div class="pay-step">
            ...
            <div class="person-check check">
                ...
                <div class="person-checkinner">
                    ...
                    <div><span>支付金额</span>：<span id="total">${sessionScope.oldForder.total}</span></div>
                </div>
            </div>
            <div class="pay">
                <div class="pay-inner">
                    <div class="fl">您的余额</div>
                    <div class="fl yibao-logo">
                        <span id="balance">${sessionScope.user.balance}</span>
                    </div>
                    <div class="fr blue aa">请确保您的余额充足
                        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a class="red"href="send2_user_recharge.action">充值</a>
                    </div>

                </div>
            </div>
            <form action="user_pay.action" method="post">
                <input id="balance_send" type="hidden" name="bal" value="${sessionScope.user.balance}">
                <input id="total_send" type="hidden" name="total" value="${sessionScope.oldForder.total}" />
                <input type="submit" style="width: 80px;float:right; height: 40px;" value="确认支付" />
            </form>
        </div>
